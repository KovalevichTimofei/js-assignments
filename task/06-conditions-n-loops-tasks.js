...
function isTriangle(a, b, c) {
    let cos = (Math.pow(c, 2) - Math.pow(a ,2) - Math.pow(b, 2)) / (2 * a * b);
...
function doRectanglesOverlap(rect1, rect2) {
    let hdif = Math.abs(rect1.top - rect2.top),
    	wdif = Math.abs(rect1.left - rect2.left),
    	hmin = rect1.top < rect2.top ? rect1.height : rect2.height,
    	wmin = rect1.left < rect2.left ? rect1.width : rect2.width;
    if (hdif < hmin && wdif < wmin)
    	return true;
    else
    	return false;
}
...
function isInsideCircle(circle, point) {
    let xPath = circle.center.x - point.x,
   		yPath = circle.center.y - point.y;
    return Math.sqrt(Math.abs(Math.pow(xPath, 2) + Math.pow(yPath, 2))) < circle.radius;
}
...
function getDigitalRoot(num) {
    do
    {
    	num = ('' + num).split('').reduce((sum, next) => sum + +next, 0);
	}
	while(num > 9);
	return num;
}
...
function isBracketsBalanced(str) {
    ...
    for (let i = 0; i < str.length; i++)
    {
    	let curChar = str.charAt(i),
    		correspChar = String.fromCharCode(curChar.charCodeAt(0)-2);
...
function getMatrixProduct(m1, m2) {
	...
	for (let i = 0; i < result.length; i++)
	{
		...
		{
			result[i][j] = cellValueCalc(m1[i], getColumn(m2, j));					
		}
	}
	return result;
	
	function cellValueCalc(m1, m2)
	{
		let result = 0;
	...
	function getColumn(matr, n)
	{
		let result = [];
		for (let i = 0; i < matr.length; i++)
		{
			result.push(matr[i][n]);
		}
		return result;
	}	
}

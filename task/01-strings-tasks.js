...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i++)
    {
    	...
    	{
    		result += String.fromCharCode(str.charCodeAt(i) + (str.charCodeAt(i) > 109 ? -13 : 13));
    	}
    	else if(str.charCodeAt(i) > 64 && str.charCodeAt(i) < 91)
    	{
    		result += String.fromCharCode(str.charCodeAt(i) + (str.charCodeAt(i) > 77 ? -13 : 13));
    	}
    	else {
    		result += str.charAt(i);
    	}   	
...
function getCardId(value) {
 	let number = '♣♦♥♠'.indexOf(value.slice(-1)) * 13;
 	switch(value.slice(0, -1))
 	{
 		...
 		default:
 		{
 			return number + +value.slice(0, -1) - 1;
 		}
 	}
